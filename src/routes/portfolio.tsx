...
function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    void loadPortfolioOverviewFn({ data: { displayCurrency } }).then((o) =>
      setOverview(o),
    )
  ...
  return (
    <>
            ...
            {!hasHoldings ? (
              ...
              <>
                <PortfolioSummaryCards
                  ...
                  fxAsOf={overview.fxAsOf}
                  fxStale={overview.fxStale}
                  totalTargetPct={totalTarget}
                />

                {overview.fxMissingPairs.length > 0 && (
                  <p className="mb-6 text-xs text-error">
                    {m.portfolio.fxPartialTotals}
                  </p>
                )}
                ...
                <PortfolioContributionSuggestions
      ...
      <div className="mx-4 mb-6 mt-2 flex items-center gap-2 rounded-xl border border-tertiary/20 bg-tertiary/8 px-4 py-3 sm:mx-8">
        <span className="material-symbols-outlined shrink-0 text-base text-tertiary">
          info
        </span>
        <p className="text-xs text-on-surface-variant">
