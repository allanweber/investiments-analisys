...
const LABEL_CLASS = 'block text-[10px] font-bold uppercase tracking-widest text-outline'
...
export function AddRendaFixaModal({ modal, rfForm }: Props) {
  ...
  const {
    ...
    allowedIndexers, fieldError, saveError, saveBlockReason, isSaving, canSave, isEdit, save,
  } = rfForm

  const isNew = form.investmentId === '__new__'
  const isFloating = form.indexer === 'cdi' || form.indexer === 'selic'
  const needsRate = form.indexer === 'pre' || form.indexer === 'ipca' || form.indexer === 'igpm'
  ...
  function handleProductTypeChange(pt: string) {
    ...
    const nextIndexer = (allowed as readonly string[]).includes(form.indexer)
      ? form.indexer
      : (allowed[0] ?? 'cdi')
    setForm((f) => ({ ...f, productType: pt, indexer: nextIndexer, annualRate: 0, multiplierPct: 100 }))
  }
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          ...
          <label className={`${LABEL_CLASS} sm:col-span-2`}>
            ...
            <select
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, investmentId: e.target.value, newName: '' }))
              }
            ...
            >
              ...
              {!isEdit && (
                <option value="__new__">{m.portfolio.rendaFixaNewInvestmentOption}</option>
              )}
            </select>
          </label>
          ...
          {isNew && (
            <>
              <div className="flex gap-2 rounded-xl border border-secondary-fixed/35 bg-secondary-fixed/12 px-3 py-2 text-xs leading-relaxed text-on-surface sm:col-span-2">
                ...
                <p>Informe o nome do novo investimento. Ele ficará disponível para pontuação.</p>
              </div>
              ...
              {typeOptions !== null && typeOptions.length === 0 && (
                <div className="flex gap-2 rounded-xl border border-error/30 bg-error/8 px-3 py-2 text-xs leading-relaxed text-on-surface sm:col-span-2">
                  <span className="material-symbols-outlined shrink-0 text-base text-error">warning</span>
                  <p>Nenhum tipo de renda fixa encontrado. Crie um tipo de investimento com a opção &ldquo;Renda fixa&rdquo; ativada antes de continuar.</p>
                </div>
              )}
          ...
          <label className={LABEL_CLASS}>
            ...
            >
              {(allowedIndexers as Indexer[]).map((idx) => (
                <option key={idx} value={idx}>{INDEXER_LABELS[idx]}</option>
              ))}
            </select>
          </label>
          ...
          {(isFloating || needsSpread) && (
            <div className="flex gap-2 rounded-xl border border-tertiary-fixed-dim/30 bg-tertiary-fixed-dim/12 px-4 py-3 text-xs leading-relaxed text-on-surface sm:col-span-2">
              <span className="material-symbols-outlined shrink-0 text-lg text-tertiary-fixed-dim">account_balance</span>
              <p>
                {form.indexer === 'cdi' ? m.portfolio.rendaFixaBcbRateCdi : m.portfolio.rendaFixaBcbRateSelic}
              </p>
            </div>
          )}
          ...
          {isFloating && (
            <PercentInput
              ...
              label={form.indexer === 'cdi' ? m.portfolio.rendaFixaMultiplierCdi : m.portfolio.rendaFixaMultiplierSelic}
              placeholder="100,00"
          ...
          {needsRate && (
            <PercentInput
              value={form.annualRate}
              label={form.indexer === 'pre' ? m.portfolio.rendaFixaAnnualRatePre : m.portfolio.rendaFixaAnnualRateSpread}
