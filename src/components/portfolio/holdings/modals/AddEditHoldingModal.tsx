...
export function AddEditHoldingModal({ modal, form, rows }: Props) {
  ...
  const existingHoldingForAdd = !isEdit
    ? findExistingHoldingForAdd(rows, f.investmentId, f.ticker)
    : undefined
  const holdingIsFixedIncome = isEdit
    ? isFixedIncomeTipo(state.row.fixedIncome, state.row.investmentTypeName)
    : Boolean(
        isFixedIncomeTipo(
          selectedOpt?.fixedIncome ?? false,
          selectedOpt?.typeName,
        ) ||
        isFixedIncomeTipo(
          existingHoldingForAdd?.fixedIncome ?? false,
          existingHoldingForAdd?.investmentTypeName,
        ),
      )
        ...
        <form
          onSubmit={(e) => {
            ...
            const canSave = isEdit
              ? invOptions !== null && Boolean(f.investmentId)
              : canSaveAddHolding
        ...
        >
          <div className="mb-6 flex items-start justify-between gap-4">
            <div className="min-w-0">
              <h3
                id="modal-add-edit-holding-title"
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            {tickerInvestHint?.type === 'loading' && (
              <p className="text-xs text-outline sm:col-span-2">
              ...
              <div className="flex flex-col gap-4 sm:col-span-2">
                ...
                <label className="block text-[10px] font-bold uppercase tracking-widest text-outline">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setForm({ ...f, investmentTypeId: e.target.value })
                    }
                ...
                <label className="block text-[10px] font-bold uppercase tracking-widest text-outline">
                  ...
                  <select
                    value={f.currency}
                    onChange={(e) =>
                      setForm({ ...f, currency: e.target.value })
                    }
          ...
          <div className="mt-8 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end sm:gap-4">
            <button
              ...
              onClick={() =>
                isEdit ? modal.close() : modal.openChooseAssetClass()
              }
            >
            ...
            <button
              ...
              disabled={
                isEdit
                  ? invOptions === null || !f.investmentId
                  : !canSaveAddHolding
              }
