...
function readStoredDisplayCurrency(): FxCurrency {
  ...
  const stored = localStorage.getItem(DISPLAY_CURRENCY_STORAGE_KEY)?.trim().toUpperCase()
  if (stored && (SUPPORTED_FX_CURRENCIES as readonly string[]).includes(stored)) {
...
export function useDisplayCurrency() {
  const [displayCurrency, setDisplayCurrencyState] = useState<FxCurrency>(readStoredDisplayCurrency)
